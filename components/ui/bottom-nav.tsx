...
import { LiquidGlassContainer, LiquidGlassIndicator } from "@/components/ui/liquid-glass-container";
...
const NAV_ITEMS: ReadonlyArray<{ tab: BottomNavTab; label: string; icon: typeof House }> = [
...
export function BottomNav({ active, onChange, hidden = false, className }: BottomNavProps): JSX.Element {
  ...
  return (
    ...
    >
      <LiquidGlassContainer className="glass-float rounded-[var(--radius-float)] p-1.5">
        <nav aria-label="メインナビゲーション" data-slot="bottom-nav" className="flex items-stretch gap-1">
            ...
            return (
              <button
                ...
                className={cn(
                  ...
                  isActive ? "text-primary-container-foreground" : "text-muted-foreground",
                )}
              >
                {isActive ? <LiquidGlassIndicator name="nav-pill" /> : null}
                <motion.span
                  className="flex flex-col items-center gap-0.5"
                  animate={reduceMotion ? undefined : { scale: isActive ? 1 : 0.96 }}
