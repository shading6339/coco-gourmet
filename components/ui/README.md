...
- MCP: shadcn MCP の `get_add_command_for_items`
...
| ファイル | 用途 |
|----------|------|
| `app-bar.tsx` | Glass 検索バー（折りたたみ / 展開）※ Issue 3 |
| `pagination.tsx` | 番号ページネーション（前へ / 次へ）※ Issue 9 |
| `typography.tsx` | 見出し・本文・ラベル |
| `search-range-tabs.tsx` | 半径タブ（Tabs ベース） |
| `animated-number.tsx` | ローリング数値カウンタ |
...
```tsx
import { Button, Typography, SearchRangeTabs } from "@/components/ui";
// または個別
